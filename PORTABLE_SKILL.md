...
13. Run `python examples/validate_portable_evaluation.py` before publishing changes to portable evaluation cases.
14. Run `python examples/validate_reference_catalog.py` before publishing reference metadata changes.
15. Run `python examples/validate_tool_catalog.py` before publishing tool metadata changes.
16. Run `python examples/validate_response_contract.py` before publishing response-contract changes.
17. Run `python examples/validate_capability_matrix.py` before publishing capability-matrix changes.
18. Run `python examples/validate_source_quality_policy.py` before publishing source-quality policy changes.
19. Use `portable-skill.json` when a platform needs a machine-readable manifest of entrypoints, tools, references, integration docs, governance files, domains, and guardrails.
20. Run `python examples/validate_portable_manifest.py` before publishing manifest changes.
21. Use `schemas/portable-skill.schema.json`, `schemas/portable-evaluation-suite.schema.json`, `schemas/reference-catalog.schema.json`, `schemas/tool-catalog.schema.json`, `schemas/response-contract.schema.json`, `schemas/capability-matrix.schema.json`, and `schemas/source-quality-policy.schema.json` when a platform needs JSON Schema validation.
